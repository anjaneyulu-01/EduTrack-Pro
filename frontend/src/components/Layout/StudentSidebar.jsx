...
export default function StudentSidebar({ open, collapsed, onClose, onToggleCollapse }) {
    ...
    `}>
        ...
        {!collapsed && (
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-500 mt-0.5 font-medium">Student Portal</p>
            </div>
          </div>
        )}
        {collapsed && (
      ...
      <div className="p-3 border-t border-gray-100 dark:border-gray-800">
        <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
          <div className="w-9 h-9 flex-shrink-0 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white text-sm font-bold">
            {initials}
          </div>
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{user?.name}</p>
              <p className="text-xs text-emerald-500 font-medium">Student</p>
            </div>
          )}
          {!collapsed && (
        ...
        {collapsed && (
          <button onClick={handleLogout} className="mt-2 w-full btn-ghost justify-center text-rose-400" title="Logout">
            <LogOut className="w-4 h-4" />
          </button>
        )}
      </div>
